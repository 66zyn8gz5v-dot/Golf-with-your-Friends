...
export function periodOf(def) { let p = 0; for (const o of (def.obstacles || [])) { if (o.period) p = Math.max(p, o.period); if (o.type === 'ferry') p = Math.max(p, 2 * ((o.wait ?? 2.5) + (o.travel ?? 3))); if (o.type === 'rotor') p = Math.max(p, o.swing ? 2 * Math.PI / o.swing.speed : 2 * Math.PI / ((o.blades || 4) * Math.abs(o.speed || 1))); if (o.type === 'windmill') p = Math.max(p, 2 * Math.PI / ((o.blades || 4) * (o.speed || 1.2))); if (o.type === 'gearlift') p = Math.max(p, 2 * Math.PI / ((o.zaehne || 6) * Math.abs(o.speed || 1.05))); if (o.type === 'hand') p = Math.max(p, 2 * Math.PI / Math.abs(o.speed || 1.05)); } return p || 6; }
