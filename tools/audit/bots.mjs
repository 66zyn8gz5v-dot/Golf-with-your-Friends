...
export function periodOf(def) { let p = 0; for (const o of (def.obstacles || [])) { if (o.period) p = Math.max(p, o.period); if (o.type === 'ferry') p = Math.max(p, 2 * ((o.wait ?? 2.5) + (o.travel ?? 3))); if (o.type === 'rotor') p = Math.max(p, o.swing ? 2 * Math.PI / o.swing.speed : 2 * Math.PI / ((o.blades || 4) * Math.abs(o.speed || 1))); if (o.type === 'windmill') p = Math.max(p, 2 * Math.PI / ((o.blades || 4) * (o.speed || 1.2))); if (o.type === 'gearlift') p = Math.max(p, 2 * Math.PI / ((o.zaehne || 8) * Math.abs(o.speed || 1.05))); if (o.type === 'hand') p = Math.max(p, 2 * Math.PI / Math.abs(o.speed || 1.05)); if (o.type === 'gearfield') p = Math.max(p, 2 * ((o.wait ?? 2.2) + (o.travel ?? 3.2))); if (o.type === 'pendulum') p = Math.max(p, 3.4); if (o.type === 'escapement') p = Math.max(p, 5.2); if (o.type === 'sweephand') p = Math.max(p, 12); if (o.type === 'handclock') p = Math.max(p, 24); if (o.type === 'dial' || o.type === 'wanderloch') p = Math.max(p, 10 * (Array.isArray(o.stellen) ? o.stellen.length : (o.marken || 12))); } return p || 6; }
...
function scoreOf(r, k, p) { if (r.last === 'sunk') return 0.02 * Math.abs(k) + 0.05 * p; if (r.last === 'max') return 10; return 1 + progress(r) / 6 + 0.03 * p + (['water', 'lava', 'oob', 'shark', 'spiked', 'zapped', 'fell'].includes(r.last) ? 1 : 0); }
